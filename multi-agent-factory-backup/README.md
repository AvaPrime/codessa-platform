...
The project uses a streamlined test matrix for fast feedback and deployment confidence:

```bash
# Quick validation (< 2 minutes) - recommended before commit
make validate-quick

# Full validation (< 10 minutes) - comprehensive check
make validate
...
```bash
make test-critical     # Auth, security, core API
```

**Security Tests** - Security-focused validation:
```bash
make test-security     # Authentication, authorization, security
```

**Integration Tests** - Cross-component testing:
```bash
make test-integration  # Requires Redis & PostgreSQL services
```

**Complete Test Suite** - Traditional approach:
```bash
make test             # Full test suite with coverage
make test-matrix      # Streamlined matrix (CI equivalent)
```
...
1. **Before committing**: `make validate-quick`
2. **Before pushing**: `make validate`  
3. **CI pipeline**: Runs streamlined matrix automatically
4. **Full validation**: `make test` for comprehensive testing

See [Streamlined Test Matrix](docs/testing/STREAMLINED_TEST_MATRIX.md) for detailed documentation.
