...
def _invalidate_search_cache() -> None:
    """
    Invalidate all search cache entries when documents are modified.
    Uses pattern matching to clear all vector search cache keys.
    """
    try:
        # Clear all vector search cache keys using the cache's clear_pattern method
        deleted_count = 0
        for pattern in ["vector_search:*", "vector_search_emb:*"]:
            deleted_count += cache.clear_pattern(pattern)
        if deleted_count > 0:
            print(f"Invalidated {deleted_count} search cache entries")
...
def upsert_document(doc_id: str, text: str, metadata: Dict[str, Any]) -> None:
    ...
    Invalidates search cache to maintain consistency.
    """
    vec = embed_text(text)
    ...
    # Invalidate search cache since new document affects search results
...
def search(query: Query, k: int = 5) -> List[Dict[str, Any]]:
    """
    Cosine search with Redis caching. Accepts either:
    ...
    if isinstance(query, str):
        ...
        # Already an embedding - create hash from embedding values
        embedding_hash = hash(tuple(query[:10]))  # Use first 10 values for hash
        cache_key = f"vector_search_emb:{embedding_hash}:{k}"
        try:
            cached_result = cache.get(cache_key)
            if cached_result is not None:
                return cached_result
    ...
    try:
        cache.set(cache_key, rows, ttl=3600)
    except Exception as e:
...
def delete_document(doc_id: str) -> bool:
    ...
    if deleted:
        # Invalidate search cache since document removal affects search results
        _invalidate_search_cache()
